...
- **Cask names must be unique across all taps.** Check existing taps (especially `opgginc/tap`) for naming conflicts before adding. If a cask with the same name exists upstream, append a suffix (e.g., `opencode-desktop-beta`).
